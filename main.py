...
#update review
@app.put("/movies/reviews/", response_model=schemas.Review)
def put_review(review: schemas.ReviewCreate, db: Session=Depends(get_db), current_user: models.User=Depends(get_current_user)):
    db_review = crud.update_review(db, current_user.user_name, review)
    if not db_review:
        raise HTTPException(status_code=404, detail="Review not found")
    return db_review

#delete review
@app.delete("/movies/reviews/")
def remove_review(review: schemas.ReviewCreate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    success = crud.delete_review(db, current_user.user_name, review)
    if not success:
        raise HTTPException(status_code=404, detail="Review not found")
